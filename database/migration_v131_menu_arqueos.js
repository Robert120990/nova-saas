...
const PERMISSION_KEY = 'view_arqueos_report';
...
async function runMigration() {
  ...
  try {
    console.log('Adding menu item: Reporte de Arqueos...');

    const [existing] = await pool.query("SELECT id FROM menu_items WHERE label = 'Reporte de Arqueos' LIMIT 1");
    ...
    const [ventasRows] = await pool.query("SELECT id FROM menu_items WHERE label = 'Ventas' AND parent_id IS NULL LIMIT 1");
    if (ventasRows.length === 0) {
      console.log('  → Ventas node not found, skipping');
      await pool.end();
      return;
    }
    const ventasId = ventasRows[0].id;

    const [parentRows] = await pool.query("SELECT id FROM menu_items WHERE label = 'Reportes' AND parent_id = ? LIMIT 1", [ventasId]);
    if (parentRows.length === 0) {
      console.log('  → Reportes (hijo de Ventas) not found, skipping');
      await pool.end();
      return;
    }
    ...
    const result = await pool.query(
      `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active, hide_in_menu)
       VALUES (?, ?, ?, ?, ?, ?, 1, 0)`,
      [parentId, 'Reporte de Arqueos', '/ventas/reportes/arqueos', 'FileText', PERMISSION_KEY, nextOrder]
    );
