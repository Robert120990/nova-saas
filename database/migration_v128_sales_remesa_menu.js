...
const PERMISSION_KEY = 'manage_sales_remesa_delivery';
...
async function runMigration() {
  ...
  try {
    console.log('Adding menu item: Entrega de Remesas (Ventas)...');

    const [existing] = await pool.query("SELECT id FROM menu_items WHERE path = '/ventas/entrega-remesas' LIMIT 1");
    ...
    const parentId = ventasRows[0].id;
    ...
    const result = await pool.query(
      `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active, hide_in_menu)
       VALUES (?, ?, ?, ?, ?, ?, 1, 0)`,
      [parentId, 'Entrega de Remesas', '/ventas/entrega-remesas', 'Handshake', PERMISSION_KEY, nextOrder]
    );
