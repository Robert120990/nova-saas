...
const PERMISSION_KEY = 'manage_sales_settings';
...
async function runMigration() {
  ...
  try {
    console.log('Adding menu item: Configuración (Ventas)...');

    const [existing] = await pool.query("SELECT id FROM menu_items WHERE path = '/ventas/configuracion' LIMIT 1");
    ...
    const result = await pool.query(
      `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active, hide_in_menu)
       VALUES (?, ?, ?, ?, ?, ?, 1, 0)`,
      [parentId, 'Configuración', '/ventas/configuracion', 'Settings', PERMISSION_KEY, nextOrder]
    );
