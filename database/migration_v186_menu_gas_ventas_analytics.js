...
const PERMISSION_KEY = 'view_gas_ventas_analytics';
...
async function runMigration() {
    ...
    try {
        console.log('Adding menu item: Ventas Analíticas y Proyección...');

        const [existing] = await pool.query("SELECT id FROM menu_items WHERE label = 'Ventas Analíticas y Proyección' LIMIT 1");
        if (existing.length > 0) {
            ...
            const result = await pool.query(
                `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active, hide_in_menu)
                 VALUES (?, ?, ?, ?, ?, ?, 1, 0)`,
                [parentId, 'Ventas Analíticas y Proyección', '/gas-station/reporte-analitico-ventas', 'TrendingUp', PERMISSION_KEY, nextOrder]
            );
