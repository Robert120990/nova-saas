...
const PERMISSION_KEY = 'view_gas_accumulated_report';
...
async function runMigration() {
  ...
  try {
    console.log('Adding menu item: Reporte Acumulado Diario...');

    const [existing] = await pool.query("SELECT id FROM menu_items WHERE label = 'Acumulado Diario' LIMIT 1");
    ...
    const result = await pool.query(
      `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active, hide_in_menu)
       VALUES (?, ?, ?, ?, ?, ?, 1, 0)`,
      [parentId, 'Acumulado Diario', '/gas-station/reporte-acumulado-diario', 'FileBarChart', PERMISSION_KEY, nextOrder]
    );
