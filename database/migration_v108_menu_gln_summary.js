...
const PERMISSION_KEY = 'view_gas_gln_summary_report';
...
async function runMigration() {
  ...
  try {
    console.log('Adding menu item: Resumen GLN Vendidos...');

    const [existing] = await pool.query("SELECT id FROM menu_items WHERE label = 'Resumen GLN Vendidos' LIMIT 1");
    ...
    const result = await pool.query(
      `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active, hide_in_menu)
       VALUES (?, ?, ?, ?, ?, ?, 1, 0)`,
      [parentId, 'Resumen GLN Vendidos', '/gas-station/reporte-resumen-gln-vendidos', 'FileBarChart', PERMISSION_KEY, nextOrder]
    );
