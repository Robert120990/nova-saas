...
async function runMigration() {
  ...
  try {
    console.log('Running migration v159 - adding "Rentabilidad de Tienda" menu item...');
    ...
    if (!parent) {
      console.error('ERROR: No se encontró el item padre "Reportes" bajo "Ventas".');
      process.exit(1);
    }
    ...
    const [existing] = await pool.query(
      ...
      ['/ventas/reportes/rentabilidad-tienda']
    );
    ...
    if (existing.length > 0) {
      ...
      await pool.query(
        `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active)
         VALUES (?, ?, ?, ?, ?, ?, TRUE)`,
        [parentId, 'Rentabilidad de Tienda', '/ventas/reportes/rentabilidad-tienda', 'TrendingUp', PERMISSION_KEY, nextOrder]
      );
    ...
    console.log(`Migration v159 completed: ${updatedCount} roles updated`);
  } catch (error) {
