...
const PARENT_LABEL = 'Libros de IVA';
const NEW_PERMISSION = 'view_iva_anexos';
const TARGET_ROLES = ['Admin'];
const CHILDREN = [
  { label: 'Anexos de IVA', path: '/iva/anexos-iva', icon: 'FileText', permission: NEW_PERMISSION, order: 4 },
];
...
async function runMigration() {
  ...
  try {
    ...
    if (roots.length === 0) {
      console.log(`${PARENT_LABEL} menu not found, skipping.`);
    } else {
    ...
    for (const role of roles) {
      if (!TARGET_ROLES.includes(role.name)) continue;
    ...
    console.log(`\nMigration complete: ${updatedCount} roles updated (targets: ${TARGET_ROLES.join(', ')})`);
  } catch (error) {
