...
const PERMISSION_KEY = 'view_gas_complementarias_report';
...
async function runMigration() {
    ...
    try {
        console.log('Adding menu item: Complementarias Emitidas...');

        const [existing] = await pool.query("SELECT id FROM menu_items WHERE label = 'Complementarias Emitidas' LIMIT 1");
        if (existing.length > 0) {
            ...
            const result = await pool.query(
                `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active, hide_in_menu)
                 VALUES (?, ?, ?, ?, ?, ?, 1, 0)`,
                [parentId, 'Complementarias Emitidas', '/gas-station/reporte-complementarias', 'FileCheck', PERMISSION_KEY, nextOrder]
            );
