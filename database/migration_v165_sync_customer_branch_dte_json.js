...
async function migrate() {
    console.log('--- Iniciando Migración v165: Sincronizar sucursales de cliente a DTEs existentes ---');
    ...
    const [sales] = await pool.query(`
        SELECT h.id as sale_id, h.codigo_generacion, h.customer_id, h.customer_branch_id,
               ...
               cb.telefono as branch_telefono,
    ...
    for (const s of sales) {
        ...
        const currentComp = json.receptor.direccion?.complemento;
        // Solo actualizar si hay cambios
        if (currentComp && currentComp.trim() === newDireccion.complemento.trim()) {
            continue;
        }

        json.receptor.direccion = newDireccion;
        if (s.branch_telefono && String(s.branch_telefono).trim()) {
            json.receptor.telefono = String(s.branch_telefono).replace(/\D/g, '').substring(0, 30);
        }
    ...
    console.log(`\nMigración v165 completada. Total DTEs actualizados: ${updated}`);
}
...
if (require.main === module) {
    migrate()
        .then(() => process.exit(0))
        .catch(err => {
            console.error('Error en migración v165:', err);
            process.exit(1);
        });
}
