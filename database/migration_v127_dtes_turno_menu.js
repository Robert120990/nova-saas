...
const PERMISSION_KEY = 'manage_dte_shift_change';
...
async function runMigration() {
  ...
  try {
    console.log('Adding menu item: DTEs por Turno...');

    const [existing] = await pool.query("SELECT id FROM menu_items WHERE path = '/ventas/dtes-turno' LIMIT 1");
    ...
    const result = await pool.query(
      `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active, hide_in_menu)
       VALUES (?, ?, ?, ?, ?, ?, 1, 0)`,
      [parentId, 'DTEs por Turno', '/ventas/dtes-turno', 'ArrowLeftRight', PERMISSION_KEY, nextOrder]
    );
