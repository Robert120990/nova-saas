...
const PERMISSION_KEY = 'view_sales_by_customer';
...
async function runMigration() {
  ...
  try {
    console.log('Running migration v129 - adding "Ventas por Cliente" menu item...');
    ...
    if (existing.length > 0) {
      ...
      await pool.query(
        `INSERT INTO menu_items (parent_id, label, path, icon, permission_key, sort_order, is_active)
         VALUES (?, ?, ?, ?, ?, ?, TRUE)`,
        [parentId, 'Ventas por Cliente', '/ventas/reportes/cliente', 'FileText', PERMISSION_KEY, nextOrder]
      );
    ...
    console.log(`Migration v129 completed: ${updatedCount} roles updated`);
  } catch (error) {
