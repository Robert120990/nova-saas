...
const PERMISSION_KEY = 'view_sales_discounts_report';
...
async function runMigration() {
    ...
    try {
        console.log('--- Migration v224: Menú y permiso para Reporte de Descuentos en Ventas ---');
        ...
        const [existing] = await pool.query(
            ...
            ['/ventas/reportes/descuentos']
        );

        if (existing.length > 0) {
            console.log('  → El item "/ventas/reportes/descuentos" ya existe en menu_items.');
            await pool.query(`
                UPDATE menu_items 
                SET parent_id = ?, label = 'Reporte de Descuentos', icon = 'BadgePercent', permission_key = ?, is_active = 1
                WHERE id = ?
            ...
            await pool.query(`
                ...
                VALUES (?, 'Reporte de Descuentos', '/ventas/reportes/descuentos', 'BadgePercent', ?, ?, 1)
        ...
        console.log(`--- Migración v224 completada: ${updatedCount} roles actualizados con ${PERMISSION_KEY} ---`);
    } catch (error) {
        console.error('Error durante la migración v224:', error);
        throw error;
    } finally {
        await pool.end();
    }
