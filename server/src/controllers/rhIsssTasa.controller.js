...
const TABLE = 'rh_isss_tasas';
const LABEL = 'Tasa de ISSS';

const getIsssTasas = async (req, res) => {
    try {
        const { search, page = 1, limit = 10 } = req.query;
        const offset = (page - 1) * limit;

        let query = `
            SELECT id, company_id, porcentaje_empleado, porcentaje_patrono, tope_quincenal, tope_mensual, created_at,
...
const createIsssTasa = async (req, res) => {
    try {
        ...
        res.status(201).json({ id: result.insertId, fecha_desde, fecha_hasta, porcentaje_empleado, porcentaje_patrono, tope_quincenal, tope_mensual });
    } catch (error) {
        if (error.code === 'ER_DUP_ENTRY') {
            return res.status(409).json({ message: `Ya existe una tasa configurada para la fecha ${fecha_desde}` });
        }
        res.status(500).json({ message: error.message });
    }
};
...
const updateIsssTasa = async (req, res) => {
    try {
        ...
        res.json({ id, fecha_desde, fecha_hasta, porcentaje_empleado, porcentaje_patrono, tope_quincenal, tope_mensual });
    } catch (error) {
        if (error.code === 'ER_DUP_ENTRY') {
            return res.status(409).json({ message: `Ya existe una tasa configurada para la fecha ${fecha_desde}` });
        }
...
const deleteIsssTasa = async (req, res) => {
    try {
        const { id } = req.params;
...
module.exports = { getIsssTasas, createIsssTasa, updateIsssTasa, deleteIsssTasa };
