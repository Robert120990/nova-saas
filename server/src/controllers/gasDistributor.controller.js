...
const TABLE = 'gas_station_distributors';
...
exports.createDistributor = async (req, res) => {
    try {
        ...
        if (error.code === 'ER_DUP_ENTRY') return res.status(400).json({ message: 'Ya existe una distribuidora con ese código' });
        res.status(500).json({ message: 'Error al crear distribuidora' });
    }
};
...
exports.updateDistributor = async (req, res) => {
    try {
        ...
        res.json({ message: 'Distribuidora actualizada' });
    } catch (error) {
        if (error.code === 'ER_DUP_ENTRY') return res.status(400).json({ message: 'Ya existe una distribuidora con ese código' });
        res.status(500).json({ message: 'Error al actualizar distribuidora' });
    }
};
...
exports.deleteDistributor = async (req, res) => {
    try {
        ...
        res.json({ message: 'Distribuidora eliminada' });
    } catch (error) {
        res.status(500).json({ message: 'Error al eliminar distribuidora' });
    }
};
