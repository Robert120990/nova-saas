...
const createSale = async (req, res) => {
    ...
    try {
        ...
        const [companies] = await connection.query(
            'SELECT dte_active, razon_social, nit, ambiente FROM companies WHERE id = ?', 
            [req.company_id]
        );
        ...
        if (company && company.dte_active) {
            const dteResult = await dteService.emitDTE(company, req.body);
            if (dteResult.success) {
                dteInfo = dteResult.data;
            } else if (!dteResult.skip) {
                throw new Error('Falla en la comunicación con Hacienda: ' + (dteResult.error || 'Error desconocido'));
            }
        }
...
const getSales = async (req, res) => {
    const { page = 1, limit = 10, dte_type, start_date, end_date, search = '' } = req.query;
    const offset = (page - 1) * limit;

    try {
        let sql = `
            SELECT h.*, s.nombre as seller_name, 
...
const getSaleById = async (req, res) => {
    ...
    try {
        const [header] = await pool.query(`
            SELECT h.*, COALESCE(c.nombre, h.cliente_nombre, 'Consumidor Final') as customer_name, 
            c.direccion as customer_address, c.nit as customer_nit
            FROM sales_headers h
            LEFT JOIN customers c ON h.customer_id = c.id
