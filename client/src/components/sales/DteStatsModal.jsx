...
const DteStatsModal = ({ isOpen, onClose }) => {
    ...
    const { data: statsData, isLoading, isError, error, refetch, isFetching } = useQuery({
        queryKey: ['sales-dte-stats', startDate, endDate, branchId],
        queryFn: async () => {
            const params = {};
    ...
    return (
        <Modal 
            isOpen={isOpen} 
            onClose={onClose} 
            title="Estadísticas de DTEs Emitidos" 
            maxWidth="max-w-6xl"
        >
            <div className="space-y-6">
                ...
                <div className="flex justify-end pt-3 border-t border-slate-100">
                    <button
                        type="button"
                        onClick={onClose}
                        className="px-6 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs uppercase tracking-wider rounded-xl transition-all"
                    >
                        Cerrar
                    </button>
                </div>
