...
const ProviderStatement = () => {
    ...
    const { data: statementData, isLoading: isLoadingStatement } = useQuery({
        queryKey: ['provider-statement', selectedProviderId, selectedBranchId, searchTerm, page],
        queryFn: async () => {
            if (!selectedProviderId || !selectedBranchId) return null;
            const res = await axios.get('/api/cxp/statement', { 
                params: { provider_id: selectedProviderId, branch_id: selectedBranchId, search: searchTerm, page, limit } 
            });
            return res.data;
        },
        enabled: !!selectedProviderId && !!selectedBranchId && activeTab === 'movimientos'
    });

    // Pestaña 2: Antigüedad de Saldos
    const { data: agingData, isLoading: isLoadingAging } = useQuery({
        queryKey: ['provider-aging', selectedProviderId, selectedBranchId],
        queryFn: async () => {
            if (!selectedProviderId || !selectedBranchId) return null;
            const res = await axios.get('/api/cxp/aging-report', { 
                params: { provider_id: selectedProviderId, branch_id: selectedBranchId } 
            });
            return res.data;
        },
        enabled: !!selectedProviderId && !!selectedBranchId && activeTab === 'antiguedad'
    });
    ...
    const handleSendEmail = async () => {
        ...
        const endpoint = activeTab === 'movimientos' 
            ? '/api/cxp/statement/send-email' 
            : '/api/cxp/aging-report/send-email';

        const toastId = toast.loading('Enviando reporte al proveedor...');
        try {
            await axios.post(endpoint, {
                provider_id: selectedProviderId,
                branch_id: selectedBranchId
            });
            toast.success('Reporte enviado exitosamente', { id: toastId });
        } catch (error) {
            toast.error(error.response?.data?.message || 'Error al enviar el correo', { id: toastId });
        }
    };
    ...
    const handleExportPDF = async () => {
        ...
        const fileName = activeTab === 'movimientos' ? 'Estado_Cuenta_Proveedor' : 'Antiguedad_Saldos_Proveedor';
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-slate-900 tracking-tighter uppercase italic">Consulta de Proveedor (CxP)</h2>
                    <p className="text-slate-500 font-medium text-xs uppercase tracking-widest">Control de deudas y antigüedad de saldos con proveedores</p>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button 
                        ...
                        title="Enviar por Correo al Proveedor"
                    >
                        <Mail size={20} />
                    </button>
                </div>
            ...
            <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 flex items-center gap-2">
                        <Truck size={12} /> Seleccionar Proveedor
                    </label>
                    <SearchableSelect 
            ...
            {/* Summary Balance */}
            {selectedProviderId && selectedBranchId && totalBalance !== undefined && (
                <div className="bg-rose-600 rounded-[1.5rem] p-5 text-white flex flex-col md:flex-row items-center justify-between gap-4 shadow-lg shadow-rose-100 italic animate-in zoom-in-95 duration-300">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center backdrop-blur-md">
                            <DollarSign size={24} />
                        </div>
                        <div>
                            <h3 className="text-[9px] font-black uppercase tracking-widest text-rose-100">Deuda Total Pendiente</h3>
            ...
            {!selectedProviderId || !selectedBranchId ? (
                <div className="bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-200 py-32 flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <div>
                        <p className="text-sm font-black text-slate-400 uppercase tracking-widest italic">Seleccione una sucursal y un proveedor para generar el reporte</p>
                    </div>
                </div>
                ...
                isLoadingStatement ? (
                    ...
                    <div className="space-y-6">
                        <div className="flex bg-white p-4 rounded-3xl border border-slate-100 shadow-sm">
                            <div className="relative w-full md:w-96">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-2xl text-[11px] font-bold uppercase outline-none focus:ring-4 focus:ring-rose-500/5 focus:border-rose-400 transition-all font-mono"
                                    value={searchTerm}
                        ...
                        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
                            <Table 
                                headers={['Fecha', 'Documento / Referencia', 'Concepto', 'Deuda (+)', 'Pagos (-)', 'Saldo']}
                                    ...
                                    <tr key={i} className="hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-0 italic">
                                        ...
                                        <td className="px-8 py-4">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] font-bold text-rose-500 font-mono tracking-tighter">{m.numero || 'S/N'}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-4">
                                            <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black tracking-widest uppercase ${
                                                m.concepto === 'COMPRA' ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'
                                            }`}>{m.concepto}</span>
                                        </td>
                    ...
                    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden animate-in fade-in lg:mx-[-1rem]">
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-slate-50 italic">
                                    {agingData.documents.map((d, i) => (
                                        <tr key={i} className="hover:bg-slate-50/30 transition-colors">
                                            ...
                                            <td className="px-6 py-3 text-[10px] font-black text-rose-500 font-mono tracking-tighter">{d.documento}</td>
                                ...
                                <tfoot className="bg-rose-50/30 border-t-2 border-rose-100/50">
                                    <tr className="font-black text-[10px] text-rose-600 italic">
                                        <td colSpan={3} className="px-6 py-4 text-xs uppercase tracking-widest text-rose-900">Totales Saldo</td>
