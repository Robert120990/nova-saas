...
const Islands = () => {
    ...
    const mutation = useMutation({
        mutationFn: (data) => {
            if (selectedItem) return axios.put(`/api/gas-station/islands/${selectedItem.id}`, data);
            return axios.post('/api/gas-station/islands', data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries(['gas-islands']);
            setIsModalOpen(false);
            setSelectedItem(null);
            toast.success(selectedItem ? 'Isla actualizada' : 'Isla creada');
        },
        onError: (error) => toast.error(error.response?.data?.message || 'Error al guardar')
    });

    const deleteMutation = useMutation({
        mutationFn: (id) => axios.delete(`/api/gas-station/islands/${id}`),
        onSuccess: () => {
            queryClient.invalidateQueries(['gas-islands']);
    ...
    const handleDelete = async (id) => {
        const ok = await confirm({
            title: '¿Eliminar isla?',
            message: 'Esta isla será eliminada permanentemente.',
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-slate-900">Islas</h2>
            ...
            <Modal
                ...
                title={selectedItem ? 'Editar Isla' : 'Nueva Isla'}
            >
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="descripcion" defaultValue={selectedItem?.descripcion} required placeholder="Nombre de la isla" className={fieldCls} />
                    </div>
