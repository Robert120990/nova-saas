...
const BalanceComprobacion = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            const response = await axios.get('/api/accounting/reports/balance-comprobacion', {
                params,
                responseType: 'blob'
            });
            ...
            setPdfUrl(url);
            toast.success('Balance de comprobación generado correctamente');
        } catch (error) {
            console.error('Error generating report:', error);
            toast.error('Error al generar el balance de comprobación');
        } finally {
    ...
    const handleDownload = () => {
        ...
        link.href = pdfUrl;
        link.setAttribute('download', `Balance_Comprobacion_${filters.modality}_${filters.year}_Mes_${filters.month}_Nivel_${filters.level}.pdf`);
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `Balance_Comprobacion_${filters.modality}_${filters.year}_Mes_${filters.month}.xlsx`);
            document.body.appendChild(link);
            link.click();
            link.remove();
    ...
    return (
        <ReportLayout
            ...
            subtitle="Resumen de sumas de cargos y abonos con saldos resultantes por nivel."
        ...
        >
            ...
            <div className="space-y-2">
                ...
                <select
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-xs font-black text-slate-700 outline-none focus:ring-4 focus:ring-indigo-500/5 transition-all"
                >
                    <option value="cargos_abonos">Cargos y Abonos (Saldo Inicial, Movimientos y Saldo Final)</option>
                    <option value="niveles">Por Niveles Jerárquicos</option>
                    <option value="cuenta">Formato Cuenta (2 Columnas: Deudoras y Activos / Acreedoras, Pasivo y Patrimonio)</option>
                </select>
            </div>
        </ReportLayout>
    );
