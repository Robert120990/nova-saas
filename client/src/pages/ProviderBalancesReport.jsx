...
const ProviderBalancesReport = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            setPdfUrl(url);
            toast.success('Reporte de saldos de proveedores generado');
        } catch (error) {
            console.error('Error generating report:', error);
            toast.error(error.message || 'Error al generar el reporte de saldos de proveedores');
        } finally {
    ...
    const handleDownload = () => {
        ...
        link.href = pdfUrl;
        link.setAttribute('download', `Saldos_Proveedores_${endDate}.pdf`);
    ...
    return (
        <ReportLayout
            ...
            subtitle="Análisis de cuentas por pagar: estados de saldos pendientes con proveedores a una fecha específica."
        ...
        >
            ...
            <div className="pt-4 p-6 bg-slate-50/50 rounded-2xl border border-slate-100 space-y-4">
                <div className="flex items-center gap-3 text-slate-400">
                    <Truck size={16} />
                    <span className="text-[10px] font-black uppercase tracking-wider">Cuentas por Pagar</span>
                </div>
                <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
                    Reporte consolidado de deudas vigentes considerando todas las compras a crédito y abonos hasta el corte.
                </p>
            </div>
        </ReportLayout>
    );
...
export default ProviderBalancesReport;
