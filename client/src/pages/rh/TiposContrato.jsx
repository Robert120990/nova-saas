...
const TiposContrato = () => {
    ...
    const { data: response = { data: [], total: 0, totalPages: 0 }, isLoading } = useQuery({
        queryKey: ['rh-tipos-contrato', debouncedSearch, page],
        queryFn: async () => (await axios.get('/api/rh/tipos-contrato', { params: { search: debouncedSearch, page } })).data
    });
    ...
    const mutation = useMutation({
        mutationFn: (data) => {
            if (selected) return axios.put(`/api/rh/tipos-contrato/${selected.id}`, data);
            return axios.post('/api/rh/tipos-contrato', data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['rh-tipos-contrato'] });
            setIsModalOpen(false);
            setSelected(null);
            toast.success(selected ? 'Tipo actualizado' : 'Tipo creado');
        },
        onError: (error) => {
    ...
    const deleteMutation = useMutation({
        mutationFn: (id) => axios.delete(`/api/rh/tipos-contrato/${id}`),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['rh-tipos-contrato'] }); toast.success('Tipo eliminado'); },
        onError: (error) => { toast.error(error.response?.data?.message || 'Error al eliminar'); }
    });

    const handleDelete = async (id) => {
        const ok = await confirm({ title: '¿Eliminar tipo?', message: 'Este tipo de contrato será eliminado permanentemente.', confirmLabel: 'Sí, eliminar', variant: 'danger' });
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-slate-900">Tipos de Contrato</h2>
                    <p className="text-slate-500 text-[11px] font-medium">Catálogo de tipos de contrato laboral</p>
                </div>
                <button onClick={() => { setSelected(null); setIsModalOpen(true); }} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-1.5 rounded-xl font-bold text-sm transition-all shadow-lg shadow-indigo-600/20 active:scale-95">
                    <Plus size={20} /><span>Nuevo Tipo</span>
                </button>
            </div>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
                        <td className="px-3 py-1">
                            <div className="flex items-center gap-2">
                                <div className="p-1 bg-indigo-50 text-indigo-600 rounded-lg"><FileSignature size={12} /></div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selected ? 'Editar Tipo de Contrato' : 'Nuevo Tipo de Contrato'}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className={lbl}>Código</label>
                        <input name="codigo" defaultValue={selected?.codigo} required placeholder="TC-001" className={cls} />
                    </div>
                    <div>
                        <label className={lbl}>Descripción</label>
                        <input name="descripcion" defaultValue={selected?.descripcion} required placeholder="Nombre del tipo de contrato" className={cls} />
                    </div>
