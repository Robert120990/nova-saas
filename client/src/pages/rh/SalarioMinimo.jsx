...
const SalarioMinimo = () => {
    ...
    const { data: response = { data: [], total: 0, totalPages: 0 }, isLoading } = useQuery({
        queryKey: ['rh-salario-minimo', debouncedSearch, page],
        queryFn: async () => (await axios.get('/api/rh/salario-minimo', { params: { search: debouncedSearch, page } })).data
    });
    ...
    const mutation = useMutation({
        mutationFn: (data) => {
            if (selected) return axios.put(`/api/rh/salario-minimo/${selected.id}`, data);
            return axios.post('/api/rh/salario-minimo', data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['rh-salario-minimo'] });
    ...
    const deleteMutation = useMutation({
        mutationFn: (id) => axios.delete(`/api/rh/salario-minimo/${id}`),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['rh-salario-minimo'] }); toast.success('Configuración eliminada'); },
        onError: (error) => { toast.error(error.response?.data?.message || 'Error al eliminar'); }
    });

    const handleDelete = async (id) => {
        const ok = await confirm({ title: '¿Eliminar configuración?', message: 'Esta configuración de salario mínimo será eliminada permanentemente.', confirmLabel: 'Sí, eliminar', variant: 'danger' });
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-slate-900">Salario Mínimo</h2>
                    <p className="text-slate-500 text-[11px] font-medium">Montos por rango de fechas</p>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <Table headers={['Vigencia Desde', 'Vigencia Hasta', 'Monto', 'Acciones']} data={items} isLoading={isLoading} renderRow={(item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
                        <td className="px-3 py-1">
                            <div className="flex items-center gap-2">
                                <div className="p-1 bg-emerald-50 text-emerald-600 rounded-lg"><Banknote size={12} /></div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selected ? 'Editar Salario Mínimo' : 'Nuevo Salario Mínimo'} maxWidth="max-w-2xl">
