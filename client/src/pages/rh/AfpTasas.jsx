...
const AfpTasas = () => {
    ...
    const { data: response = { data: [], total: 0, totalPages: 0 }, isLoading } = useQuery({
        queryKey: ['rh-afp-tasas', debouncedSearch, page],
        queryFn: async () => (await axios.get('/api/rh/afp-tasas', { params: { search: debouncedSearch, page } })).data
    });

    const { data: afpsResponse } = useQuery({
        queryKey: ['rh-afps-all'],
        queryFn: async () => (await axios.get('/api/rh/afps', { params: { limit: 100 } })).data
    });

    const afps = afpsResponse?.data || [];

    const items = response.data || [];

    const mutation = useMutation({
        mutationFn: (data) => {
            if (selected) return axios.put(`/api/rh/afp-tasas/${selected.id}`, data);
            return axios.post('/api/rh/afp-tasas', data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['rh-afp-tasas'] });
            setIsModalOpen(false);
            setSelected(null);
            toast.success(selected ? 'Tasa de AFP actualizada' : 'Tasa de AFP creada');
        },
        onError: (error) => {
            toast.error(error.response?.data?.message || 'Error al guardar tasa de AFP');
        }
    });

    const deleteMutation = useMutation({
        mutationFn: (id) => axios.delete(`/api/rh/afp-tasas/${id}`),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['rh-afp-tasas'] }); toast.success('Tasa eliminada'); },
        onError: (error) => { toast.error(error.response?.data?.message || 'Error al eliminar'); }
    });

    const handleDelete = async (id) => {
        const ok = await confirm({ title: '¿Eliminar tasa?', message: 'Esta tasa de AFP será eliminada permanentemente.', confirmLabel: 'Sí, eliminar', variant: 'danger' });
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-slate-900">Porcentajes AFP</h2>
            ...
            <div className="relative max-w-sm">
                ...
                <input type="text" placeholder="Buscar AFP..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-3 py-1.5 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-400 transition-all text-xs font-medium shadow-sm" />
            </div>

            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <Table headers={['AFP', 'Vigencia Desde', 'Vigencia Hasta', '% Empleado', '% Patrono', 'Tope Quincenal', 'Tope Mensual', 'Acciones']} data={items} isLoading={isLoading} renderRow={(item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
                        <td className="px-3 py-1">
                            <div className="flex items-center gap-2">
                                <div className="p-1 bg-indigo-50 text-indigo-600 rounded-lg"><BadgePercent size={12} /></div>
                                <span className="font-bold text-xs text-slate-900">{item.afp_descripcion}</span>
                            </div>
                        </td>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selected ? 'Editar Tasa de AFP' : 'Nueva Tasa de AFP'} maxWidth="max-w-3xl">
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="md:col-span-2">
                            <label className={lbl}>AFP</label>
                            <select name="afp_id" defaultValue={selected?.afp_id} required className={cls}>
                                <option value="">Seleccionar AFP...</option>
                                {afps.map(afp => (
                                    <option key={afp.id} value={afp.id}>{afp.codigo} - {afp.descripcion}</option>
                                ))}
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input name="porcentaje_empleado" type="number" step="0.01" min="0" max="100" defaultValue={selected?.porcentaje_empleado} required placeholder="7.25" className={cls} />
                        </div>
                        <div>
                            <label className={lbl}>% Patrono</label>
                            <input name="porcentaje_patrono" type="number" step="0.01" min="0" max="100" defaultValue={selected?.porcentaje_patrono} required placeholder="8.75" className={cls} />
                        </div>
                        <div>
