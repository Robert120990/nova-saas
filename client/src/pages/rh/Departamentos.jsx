...
const Departamentos = () => {
    ...
    const { data: response = { data: [], total: 0, totalPages: 0 }, isLoading } = useQuery({
        queryKey: ['rh-departamentos', debouncedSearch, page],
        queryFn: async () => (await axios.get('/api/rh/departamentos', { params: { search: debouncedSearch, page } })).data
    });
    ...
    const mutation = useMutation({
        mutationFn: (data) => {
            if (selected) return axios.put(`/api/rh/departamentos/${selected.id}`, data);
            return axios.post('/api/rh/departamentos', data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['rh-departamentos'] });
            setIsModalOpen(false);
            setSelected(null);
            toast.success(selected ? 'Departamento actualizado' : 'Departamento creado');
        },
        onError: (error) => {
    ...
    const deleteMutation = useMutation({
        mutationFn: (id) => axios.delete(`/api/rh/departamentos/${id}`),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['rh-departamentos'] }); toast.success('Departamento eliminado'); },
        onError: (error) => { toast.error(error.response?.data?.message || 'Error al eliminar'); }
    });

    const handleDelete = async (id) => {
        const ok = await confirm({ title: '¿Eliminar departamento?', message: 'Este departamento será eliminado permanentemente.', confirmLabel: 'Sí, eliminar', variant: 'danger' });
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-slate-900">Departamentos de Personal</h2>
                    <p className="text-slate-500 text-[11px] font-medium">Departamentos o áreas de la empresa</p>
                </div>
                <button onClick={() => { setSelected(null); setIsModalOpen(true); }} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-1.5 rounded-xl font-bold text-sm transition-all shadow-lg shadow-indigo-600/20 active:scale-95">
                    <Plus size={20} /><span>Nuevo Departamento</span>
                </button>
            </div>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
                        <td className="px-3 py-1">
                            <div className="flex items-center gap-2">
                                <div className="p-1 bg-indigo-50 text-indigo-600 rounded-lg"><Building2 size={12} /></div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selected ? 'Editar Departamento' : 'Nuevo Departamento'}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className={lbl}>Código</label>
                        <input name="codigo" defaultValue={selected?.codigo} required placeholder="DEP-001" className={cls} />
                    </div>
                    <div>
                        <label className={lbl}>Descripción</label>
                        <input name="descripcion" defaultValue={selected?.descripcion} required placeholder="Nombre del departamento" className={cls} />
                    </div>
                    <div className="flex justify-end gap-3 pt-4">
                        ...
                        <button type="submit" disabled={mutation.isPending} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-xl font-bold transition-all shadow-lg shadow-indigo-600/20 disabled:opacity-50">
                            {mutation.isPending ? 'Guardando...' : (selected ? 'Guardar Cambios' : 'Registrar Departamento')}
                        </button>
                    </div>
                </form>
            </Modal>
