...
const Afps = () => {
    ...
    const { data: response = { data: [], total: 0, totalPages: 0 }, isLoading } = useQuery({
        queryKey: ['rh-afps', debouncedSearch, page],
        queryFn: async () => (await axios.get('/api/rh/afps', { params: { search: debouncedSearch, page } })).data
    });
    ...
    const mutation = useMutation({
        mutationFn: (data) => {
            if (selected) return axios.put(`/api/rh/afps/${selected.id}`, data);
            return axios.post('/api/rh/afps', data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['rh-afps'] });
            setIsModalOpen(false);
            setSelected(null);
            toast.success(selected ? 'AFP actualizada' : 'AFP creada');
        },
        onError: (error) => {
            toast.error(error.response?.data?.message || 'Error al guardar AFP');
        }
    });

    const deleteMutation = useMutation({
        mutationFn: (id) => axios.delete(`/api/rh/afps/${id}`),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['rh-afps'] }); toast.success('AFP eliminada'); },
        onError: (error) => { toast.error(error.response?.data?.message || 'Error al eliminar'); }
    });

    const handleDelete = async (id) => {
        const ok = await confirm({ title: '¿Eliminar AFP?', message: 'Esta AFP será eliminada permanentemente.', confirmLabel: 'Sí, eliminar', variant: 'danger' });
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-slate-900">AFPs</h2>
                    <p className="text-slate-500 text-[11px] font-medium">Administradoras de Fondos de Pensiones</p>
                </div>
                <button onClick={() => { setSelected(null); setIsModalOpen(true); }} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-1.5 rounded-xl font-bold text-sm transition-all shadow-lg shadow-indigo-600/20 active:scale-95">
                    <Plus size={20} /><span>Nueva AFP</span>
                </button>
            </div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selected ? 'Editar AFP' : 'Nueva AFP'}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className={lbl}>Código</label>
                        <input name="codigo" defaultValue={selected?.codigo} required placeholder="AFP-001" className={cls} />
                    </div>
                    <div>
                        <label className={lbl}>Descripción</label>
                        <input name="descripcion" defaultValue={selected?.descripcion} required placeholder="Nombre de la AFP" className={cls} />
                    </div>
                    <div className="flex justify-end gap-3 pt-4">
                        ...
                        <button type="submit" disabled={mutation.isPending} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-xl font-bold transition-all shadow-lg shadow-indigo-600/20 disabled:opacity-50">
                            {mutation.isPending ? 'Guardando...' : (selected ? 'Guardar Cambios' : 'Registrar AFP')}
                        </button>
                    </div>
                </form>
            </Modal>
