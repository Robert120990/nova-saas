...
const PurchaseCheckReport = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            const response = await axios.get('/api/purchases/checks/reports/pdf', {
                params,
                responseType: 'blob'
            });
            ...
            setPdfUrl(url);
            toast.success('Reporte de cheques de contado generado correctamente');
        } catch (error) {
            console.error('Error generating report:', error);
            toast.error('Error al generar el reporte de cheques de contado');
        } finally {
    ...
    const handleDownload = () => {
        ...
        link.href = pdfUrl;
        link.setAttribute('download', `Reporte_Cheques_Contado_${filters.start_date}_al_${filters.end_date}.pdf`);
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `Reporte_Cheques_Contado_${filters.start_date}_al_${filters.end_date}.xlsx`);
            document.body.appendChild(link);
            link.click();
            link.remove();
    ...
    return (
        <ReportLayout
            title="Reporte de Cheques de Contado"
            subtitle="Cheques emitidos por rango de fecha, sucursal, destino y estado."
            category="Compras"
            pdfUrl={pdfUrl}
        ...
        >
            ...
            <div className="space-y-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <CheckCircle2 size={12} className="text-indigo-500" /> Estado
                </label>
                <select 
                    name="status"
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-xs font-black text-slate-700 outline-none focus:ring-4 focus:ring-indigo-500/5 transition-all"
