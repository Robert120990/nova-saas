...
const GasPosTypes = () => {
    ...
    const filtered = items.filter(i => !searchTerm || i.nombre.toLowerCase().includes(searchTerm.toLowerCase()));

    const mutation = useMutation({
        mutationFn: (data) => {
            if (selectedItem) return axios.put(`/api/gas-station/pos-types/${selectedItem.id}`, data);
            return axios.post('/api/gas-station/pos-types', data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries(['gas-pos-types']);
            setIsModalOpen(false);
            setSelectedItem(null);
            toast.success(selectedItem ? 'Tipo de POS actualizado' : 'Tipo de POS creado');
        },
        onError: (error) => toast.error(error.response?.data?.message || 'Error al guardar')
    });

    const deleteMutation = useMutation({
        mutationFn: (id) => axios.delete(`/api/gas-station/pos-types/${id}`),
        onSuccess: () => {
            queryClient.invalidateQueries(['gas-pos-types']);
            toast.success('Tipo de POS eliminado');
        }
    });

    const handleDelete = async (id) => {
        const ok = await confirm({
            title: '¿Eliminar tipo de POS?',
            message: 'Este tipo de POS será eliminado permanentemente.',
    ...
    return (
        <div className="space-y-3">
                        ...
                        <tr key={item.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
                            <td className="px-3 py-1">
                                <div className="flex items-center gap-2">
                                    <div className="p-1 bg-indigo-50 text-indigo-600 rounded-lg"><Monitor size={12}/></div>
            ...
            <Modal
                ...
                title={selectedItem ? 'Editar Tipo de POS' : 'Nuevo Tipo de POS'}
            >
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            required
                            placeholder="Ej: Datafono Banrural, POS BAC..."
                            className={fieldCls}
                        />
                    </div>
