...
export default function GasStationConfig() {
    ...
    useEffect(() => {
        if (settings?.lubricant_category_id) {
            setLubricantCategoryId(settings.lubricant_category_id);
        }
        if (settings?.variacion_permitida) {
            setVariacionPermitida(settings.variacion_permitida);
        }
    }, [settings]);
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="0.00"
                        className="w-full border border-slate-300 rounded-xl px-3 py-2 text-[13px] font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-400 font-mono"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">
