...
const EggCostsMaintenance = () => {
    ...
    // Returnables state (Roxy - Operaciones)
    const [returnables, setReturnables] = useState([]);
    const [loadingReturnables, setLoadingReturnables] = useState(false);
    const [movementModal, setMovementModal] = useState(null); // returnable item to add movement to
    ...
    return (
        <div className="space-y-6">
            {/* Header Banner - Shortcut to full Costeo por Libra */}
            <div className="bg-gradient-to-r from-emerald-950/60 via-slate-900 to-indigo-950/60 border border-emerald-500/30 rounded-3xl p-5 shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-white">Nuevo Módulo de Costeo por Libra (Excel Oficial)</span>
                        </div>
                        <p className="text-[11px] text-slate-400 mt-0.5">
                            Simulador paramétrico: Costeo Base por Libra, Químicos CIP, Empaques y Acuerdos Comerciales por Cliente.
                        </p>
                    </div>
                </div>
                <button
                    onClick={() => navigate('/industrial/costeo-libra')}
                    className="w-full md:w-auto px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-emerald-600/20 transition-all flex items-center justify-center gap-2 whitespace-nowrap border border-emerald-400/30"
                >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-black text-white uppercase tracking-wider">Gestión de Costos, Envases y Mantenimiento</h1>
                        <p className="text-[12px] text-slate-400 font-semibold tracking-tight">Centros de costo por lote, control logístico de cubetas retornables (Roxy), bitácora de maquinaria y previsión</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 p-1.5 bg-slate-950 rounded-2xl border border-slate-900 w-fit">
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${
                        activeTab === 'returnables' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/15' : 'text-slate-400 hover:text-slate-200'
                    }`}
                >
                    <RotateCcw size={14} />
                    Envases Retornables (Roxy)
                </button>
                <button
                ...
                <div className="space-y-4">
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-6 gap-2 mb-2">
                            <div className="bg-slate-950 border border-slate-850 rounded-xl p-3 text-center">
                                <span className="text-[9px] font-black text-slate-500 uppercase block truncate">Costo Fijo Base</span>
                            ...
                            {(() => {
                                ...
                                return (
                                    <>
                                        <div className="bg-slate-950 border border-slate-850 rounded-xl p-3 text-center">
                                            <span className="text-[9px] font-black text-slate-500 uppercase block truncate">Lotes Procesados</span>
                                            <span className="text-xs font-black text-white block mt-1">{filtered.length}</span>
                                        </div>
                                        <div className="bg-slate-950 border border-slate-850 rounded-xl p-3 text-center">
                                            <span className="text-[9px] font-black text-slate-500 uppercase block truncate">Rendimiento Líquido</span>
                                            <span className="text-xs font-black text-teal-400 block mt-1">{totalYield.toLocaleString()} Lbs</span>
                                        </div>
                                        <div className="bg-slate-950 border border-slate-850 rounded-xl p-3 text-center">
                                            <span className="text-[9px] font-black text-slate-500 uppercase block truncate">Desperdicio Cáscara</span>
                                            <span className="text-xs font-black text-rose-400 block mt-1">{totalWasteLbs.toLocaleString()} Lbs</span>
                                        </div>
                                        <div className="bg-slate-950 border border-slate-850 rounded-xl p-3 text-center">
                                            <span className="text-[9px] font-black text-slate-500 uppercase block truncate">Costo Total Acum.</span>
                                            <span className="text-xs font-black text-amber-400 block mt-1">
                                                <Money value={totalAll} />
                                            </span>
                                        </div>
                                        <div className="bg-slate-950 border border-slate-850 rounded-xl p-3 text-center">
                                            <span className="text-[9px] font-black text-slate-500 uppercase block truncate">Costo Prom./Lb</span>
                                            <span className="text-xs font-black text-indigo-400 block mt-1">
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
                        <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                            ...
                            Simulador Rápido de Precio de Venta
                        </h2>
                        <div className="h-px bg-slate-800" />
                        <div className="space-y-3">
                            ...
                            {(() => {
                                ...
                                return (
                                    <div className="bg-slate-950 border border-slate-850 rounded-xl p-3 grid grid-cols-2 gap-2 text-center">
                                        <div>
                                            <span className="text-[8px] font-black text-slate-500 block">Costo Prom./Lb</span>
                                            <span className="text-xs font-bold text-white block mt-0.5">
                                                <Money value={avgCostPerLb} />
                                            </span>
                                        </div>
                                        <div>
                                            <span className="text-[8px] font-black text-slate-500 block">Precio Venta Sug./Lb</span>
                                            <span className="text-xs font-black text-teal-400 block mt-0.5">
                                                <Money value={suggestedPrice} />
                                            </span>
                                        </div>
                                    </div>
                                );
            ...
            {/* TAB: ENVASES RETORNABLES (ROXY - OPERACIONES) */}
            {activeTab === 'returnables' && (
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                                ...
                                Control de Cubetas y Tapaderas Retornables
                            </h2>
                            <p className="text-[11px] text-slate-400 mt-1">
                                Seguimiento de saldo de envases entregados a clientes (PriceSmart, La Francesa, Lorena, etc.) y devoluciones a planta
                            </p>
                        </div>
                        <div className="flex items-center gap-2">
                            <button
                            ...
                            >
                                <RefreshCw size={14} className={loadingReturnables ? 'animate-spin' : ''} />
                            </button>
                            <button
                                onClick={() => setNewCustomerModal(true)}
                                className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-emerald-600/20 border border-emerald-500/30 transition-all"
                            >
                                <Plus size={14} />
                                Registrar Cliente / Envase
                            </button>
                        </div>
                    </div>
                                ...
                                <>
                                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Cubetas en Clientes</span>
                                            <Package size={16} className="text-amber-400" />
                                        </div>
                                        <div className="text-2xl font-black text-amber-400 mt-2">{totalPending.toLocaleString()}</div>
                                        <span className="text-[9px] text-slate-500 font-semibold">Pendientes de devolución</span>
                                    </div>

                                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Total Entregadas</span>
                                            <ArrowUpRight size={16} className="text-indigo-400" />
                                        </div>
                                        <div className="text-2xl font-black text-white mt-2">{totalDelivered.toLocaleString()}</div>
                                        <span className="text-[9px] text-slate-500 font-semibold">Con producto pasteurizado</span>
                                    </div>

                                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Total Retornadas</span>
                                            <ArrowDownRight size={16} className="text-teal-400" />
                                        </div>
                                        <div className="text-2xl font-black text-teal-400 mt-2">{totalReturned.toLocaleString()}</div>
                                        <span className="text-[9px] text-slate-500 font-semibold">Limpias / ingresadas a planta</span>
                                    </div>

                                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Tasa de Recuperación</span>
                                            <CheckCircle2 size={16} className="text-emerald-400" />
                                        </div>
                                        <div className="text-2xl font-black text-emerald-400 mt-2">{returnRate}%</div>
                                        <span className="text-[9px] text-slate-500 font-semibold">Eficiencia de retorno</span>
                                    </div>
                                </>
                            );
                        })()}
                    ...
                    {/* Table of Returnable Packaging by Customer */}
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                                Listado de Saldos por Cliente ({returnables.length})
                            </span>
                            <span className="text-[10px] text-slate-500 font-semibold">Envase estándar: Cubeta 30 lb con tapadera hermética</span>
                        </div>

                        {returnables.length === 0 ? (
                            <div className="p-8 text-center bg-slate-950 rounded-2xl border border-slate-850 text-slate-500 text-xs font-semibold">
                                No se han registrado clientes para control de envases retornables. Haga clic en "Registrar Cliente / Envase" para comenzar.
                            </div>
                        ) : (
                            <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950">
                                <table className="w-full text-left text-xs border-collapse">
                                    <thead>
                                        <tr className="bg-slate-900/60 border-b border-slate-850 text-slate-400 font-extrabold uppercase tracking-tighter text-[9px]">
                                            ...
                                            return (
                                                <tr key={r.id} className="hover:bg-slate-900/40 transition-colors">
                                                    <td className="px-4 py-3">
                                                        <div className="flex flex-col">
                                                            <span className="font-bold text-white text-[12px]">{r.customer_name}</span>
                                                            {r.customer_full_name && (
                                                                <span className="text-[9px] text-slate-500">{r.customer_full_name}</span>
                                                            )}
                                                        </div>
                                                    </td>
                                                    <td className="px-3 py-3">
                                                        <span className="px-2 py-0.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 text-[10px] font-bold uppercase">
                                                            {r.packaging_type === 'cubeta_30lb' ? 'Cubeta 30 Lb' : r.packaging_type}
                                                        </span>
                                                    </td>
                                                    <td className="px-3 py-3 text-right text-[11px] text-slate-400 font-semibold">
                                                        {r.initial_balance || 0}
                                                    </td>
                                                    <td className="px-3 py-3 text-right text-[11px] text-indigo-400 font-bold">
                                                        +{r.total_delivered || 0}
                                                    </td>
                                                    <td className="px-3 py-3 text-right text-[11px] text-teal-400 font-bold">
                                                        -{r.total_returned || 0}
                                                    </td>
                                                    ...
                                                    <td className="px-3 py-3 text-[10px] text-slate-400 font-semibold">
                                                    ...
                                                    <td className="px-4 py-3 text-center">
                                                        <button
                                                            ...
                                                            className="px-3 py-1.5 bg-indigo-600/80 hover:bg-indigo-600 text-white rounded-lg text-[10px] font-bold transition-all shadow-md shadow-indigo-600/20"
                                                        >
                                                            Movimiento &plusmn;
                                                        </button>
                                                    </td>
            ...
            {/* TAB: MANTENIMIENTO DE MAQUINARIA */}
            ...
            {activeTab === 'forecasting' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {/* Historical Chart Represented with customized CSS */}
            ...
            {/* MODAL: REGISTRAR MOVIMIENTO DE RETORNABLES */}
            {movementModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl max-w-md w-full space-y-4">
                        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                            <div>
                                <h3 className="text-sm font-bold text-white uppercase">Movimiento de Envases</h3>
                                <p className="text-[10px] text-slate-400 font-semibold">{movementModal.customer_name}</p>
                            </div>
                            <button onClick={() => setMovementModal(null)} className="text-slate-500 hover:text-white">
                                <XCircle size={18} />
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleMovementSubmit} className="space-y-4">
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Tipo de Movimiento</label>
                                <select
                                ...
                                >
                                    <option value="entrega">Entrega a Cliente (+ Entrega con Producto)</option>
                                    <option value="devolucion">Devolución a Planta (- Retorno de Vacíos)</option>
                                </select>
                            </div>

                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Cantidad de Envases (Cubetas)</label>
                                <input
                                    type="number"
                                    min="1"
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Documento / Guía / Factura</label>
                                <input
                                    type="text"
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Observaciones / Notas</label>
                                <textarea
                                    value={movementForm.notes}
                                    onChange={(e) => setMovementForm({ ...movementForm, notes: e.target.value })}
                                    placeholder="Estado de las cubetas, con o sin tapadera..."
                                    className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white font-semibold focus:outline-none h-16"
                                />
                            </div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button
                                    ...
                                    className="w-1/2 py-2.5 bg-slate-800 text-slate-300 rounded-xl text-xs font-bold hover:bg-slate-700"
                                >
                                    Cancelar
                                </button>
                                <button
            ...
            {newCustomerModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl max-w-md w-full space-y-4">
                        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                            <h3 className="text-sm font-bold text-white uppercase">Registrar Cliente para Retornables</h3>
                            <button onClick={() => setNewCustomerModal(false)} className="text-slate-500 hover:text-white">
                                <XCircle size={18} />
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleNewCustomerSubmit} className="space-y-4">
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Nombre del Cliente / Empresa</label>
                                <input
                                    type="text"
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Tipo de Empaque</label>
                                <select
                                    value={newCustomerForm.packaging_type}
                                    onChange={(e) => setNewCustomerForm({ ...newCustomerForm, packaging_type: e.target.value })}
                                    className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white font-semibold focus:outline-none"
                                >
                                    <option value="cubeta_30lb">Cubeta 30 Lb con tapadera</option>
                                    <option value="cubeta_15lb">Cubeta 15 Lb con tapadera</option>
                                    <option value="tarima_plastica">Tarima Plástica Lavable</option>
                                </select>
                            </div>

                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Saldo Inicial (Cubetas actualmente en su poder)</label>
                                <input
                                    type="number"
                                    min="0"
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Notas de Control</label>
                                <textarea
                                    value={newCustomerForm.notes}
                                    onChange={(e) => setNewCustomerForm({ ...newCustomerForm, notes: e.target.value })}
                                    placeholder="Contacto de entrega, sucursal o condición especial..."
                                    className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white font-semibold focus:outline-none h-16"
                                />
                            </div>
            ...
            {variableCostsModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl max-w-md w-full mx-4 space-y-4 max-h-[80vh] overflow-y-auto">
                        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                            <h3 className="text-sm font-bold text-white uppercase">Costos Variables por Lote</h3>
                        ...
                        <div className="space-y-2">
                            <div className="text-[9px] font-black text-slate-500 uppercase">Costos Fijos Asignados</div>
                            {costConcepts.map(cc => (
                            ...
                            <div className="flex gap-2 pt-2">
                                <input
                                    ...
                                    className="flex-1 px-2 py-1.5 bg-slate-950 border border-slate-850 rounded-lg text-[10px] text-white font-semibold focus:outline-none"
                                />
                                <div className="w-24">
                                    <MoneyInput
                                        ...
                                        placeholder="0.00"
                                        className="w-full px-2 py-1.5 bg-slate-950 border border-slate-850 rounded-lg text-[10px] text-white font-semibold text-right focus:outline-none"
                                        step="0.01"
                                    />
                                </div>
