...
const GasDespachadores = () => {
    ...
    const mutation = useMutation({
        mutationFn: (data) => {
            if (selectedItem) return axios.put(`/api/gas-station/despachadores/${selectedItem.id}`, data);
            return axios.post('/api/gas-station/despachadores', data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries(['gas-despachadores']);
            setIsModalOpen(false);
            setSelectedItem(null);
            toast.success(selectedItem ? 'Despachador actualizado' : 'Despachador creado');
        },
        onError: (error) => toast.error(error.response?.data?.message || 'Error al guardar')
    });

    const deleteMutation = useMutation({
        mutationFn: (id) => axios.delete(`/api/gas-station/despachadores/${id}`),
        onSuccess: () => {
            queryClient.invalidateQueries(['gas-despachadores']);
    ...
    const handleDelete = async (id) => {
        const ok = await confirm({
            title: '¿Eliminar despachador?',
            message: 'Este despachador será eliminado permanentemente.',
    ...
    return (
        <div className="space-y-3">
                        ...
                        <tr key={item.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
                            <td className="px-3 py-1">
                                <div className="flex items-center gap-2">
                                    <div className="p-1 bg-indigo-50 text-indigo-600 rounded-lg"><User size={12}/></div>
            ...
            <Modal
                ...
                title={selectedItem ? 'Editar Despachador' : 'Nuevo Despachador'}
            >
...
export default GasDespachadores;
