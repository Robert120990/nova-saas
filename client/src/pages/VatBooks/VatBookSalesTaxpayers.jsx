...
const VatBookSalesTaxpayers = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            const response = await axios.get('/api/vat-books/sales-taxpayers-pdf', {
                params: filters,
                responseType: 'blob'
            });
            ...
            setPdfUrl(url);
            toast.success('Libro de ventas a contribuyentes generado correctamente');
        } catch (error) {
    ...
    const handleDownload = () => {
        ...
        link.setAttribute('download', `Libro_Ventas_Contribuyentes_${scope}_${filters.month}_${filters.year}.pdf`);
    ...
    const handleExportExcel = async () => {
        try {
            ...
            link.href = url;
            const scope = filters.branch_id === 'all' ? 'Contribuyente' : `Sucursal_${filters.branch_id}`;
            link.setAttribute('download', `Libro_Ventas_Contribuyentes_${scope}.xlsx`);
            document.body.appendChild(link);
            link.click();
            link.remove();
    ...
    return (
        <ReportLayout
            ...
            subtitle="Registro de Comprobantes de Crédito Fiscal (CCF) emitidos."
...
export default VatBookSalesTaxpayers;
